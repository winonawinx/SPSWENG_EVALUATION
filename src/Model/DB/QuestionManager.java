...
public class QuestionManager
{
	...
	public ArrayList<Question> getOfficeQuestions(int officeid)
	{
		...
		try 
		{
			String query = "SELECT * FROM Questions WHERE questionID IN (Select questionID FROM formquestions WHERE FormID IN (SELECT FormID from Forms WHERE serviceID IN (select serviceID from services Where officeID = ?)))";
			statement = connect.getConnection().prepareStatement(query);
